...
def _assert_exactly_one_event(
    ...
    *,
    ...
    assert abs(payload.response_cost - cost_anchor) < 1e-12, (
...
class TestDataDogLogDelivery:
    ...
    def test_chat_completions_stream_emits_one_log_event(
        ...
        """One successful STREAMED /chat/completions call must reach the
        DataDog logs intake as exactly one log event whose payload carries the
        model, the token counts aggregated across the stream, stream=true, and
        a response cost equal to the /spend/logs row for the same call (a
        stream's headers ship before its cost exists, so the spend row is the
        cross-check anchor)."""
        _assert_datadog_configured(client)
    ...
    def test_messages_stream_emits_one_log_event(
        ...
        """One successful STREAMED /v1/messages call must reach the DataDog
        logs intake as exactly one log event whose payload carries the model,
        the token counts aggregated across the stream, stream=true, and a
        response cost equal to the /spend/logs row for the same call.
    ...
    def test_responses_stream_emits_one_log_event(
        ...
        """One successful STREAMED /v1/responses call must reach the DataDog
        logs intake as exactly one log event whose payload carries the model,
        the token counts aggregated across the stream, stream=true, and a
        response cost equal to the /spend/logs row for the same call."""
        _assert_datadog_configured(client)
